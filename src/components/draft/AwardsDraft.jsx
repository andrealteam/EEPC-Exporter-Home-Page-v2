...
const AwardsDraft = ({ memberId }) => {
  ...
  const handleSave = async () => {
    ...
    let res = null;
    if (editedHeader.id) {
      res = await updateAwards(
        editedHeader.id,
        memberId,
        editedHeader.name,
        editedHeader.awardsFile
      );
    } else {
      res = await postAwards(
        memberId,
        editedHeader.name,
        editedHeader.awardsFile
      );
    ...
    if (res.status) {
      ...
      queryClient.invalidateQueries(["awards-draft"]);
      setAddProduct(false);
      setEditedHeader({
        id: "",
        awards: "",
  ...
  const updateSectionView = async () => {
    ...
    if (res.status) {
      toast.success(res?.message);
      queryClient.invalidateQueries(["get-section-in-awards"]);
  ...
  useEffect(() => {
    if (Array.isArray(sectionData) && sectionData.includes(9)) {
      setSectionItem(true);
    } else {
                            ...
                            if (file) {
                              ...
                              reader.onload = (event) => {
                                setEditedHeader((prev) => ({
                                  ...prev,
                                  awards: event.target.result, // base64 preview
                                  awardsFile: file, // actual file to upload later
                                }));
                              };
