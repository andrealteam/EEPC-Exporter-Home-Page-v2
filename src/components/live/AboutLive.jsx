...
import { getAbout } from "../../services/draftApi";
import { useQuery } from "@tanstack/react-query";
import DOMPurify from "dompurify";
import { baseFileURL } from "../../services/api";
import { getLiveAbout, getLiveWhoWeAre } from "../../services/liveApi";
import Skeleton from "react-loading-skeleton";

const AboutLive = ({ website_url }) => {
  ...
  const {
    ...
    queryKey: ["about-live", website_url],
    queryFn: () => getLiveWhoWeAre(website_url),
  });
  ...
  const truncatedHTML =
    aboutData?.about_content
      ?.replace(/(<([^>]+)>)/gi, " ")   // remove tags TEMPORARILY
      .split(/\s+/)
      .slice(0, 300)
      .join(" ") + " ...";
  ...
  return (
    <section style={{ position: "relative", paddingTop: "60px" }}>
      <div class="row">
        <div class="col-lg-6">
          <div class="company-card random-class">
              ...
              <div>
                ...
                {isLong && (
                  <button
                    ...
                    style={{ marginTop: "10px", cursor: "pointer", background: "none", border: "none" }}
                  >
