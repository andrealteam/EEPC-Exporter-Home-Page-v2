...
import { getLiveGallery } from "../../services/liveApi";

const GalleryLive = ({ website_url }) => {
  const {
    data: productsData = [],
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["gallery-live-data", website_url],
    queryFn: async () => {
      const res = await getLiveGallery(website_url);
  ...
  if (isLoading || isError || !productsData || productsData?.length <= 0) {
    return null;
  }
